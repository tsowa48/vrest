...
if($method === 'GET') {//Read
  ...
  while($item = pg_fetch_row($items)) {
    $json .= '{"id":'.$item[0].', "name":"'.$item[1].'", "description":"'.str_replace('"', '\\"', $item[2]).'", "position":'.$item[3].', "vid":'.$item[4].'},'.PHP_EOL;
  }
  if(strlen($json) > 0)
  ...
  if(isset($position) && is_numeric($position) &&
    ...
    if($currentTime > intval($_start))
      ...
      echo '[{"id":'.$id.', "name":"'.htmlspecialchars($name).'", "description":"'.str_replace('"', '\\"', htmlspecialchars($description)).'", "position":'.$position.', "vid":'.$vid.'}]';
